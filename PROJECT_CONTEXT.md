...
1. ~~**Add `middleware.ts` — server-side auth guard**~~ ✅ Done — `proxy.ts` covers `/coach/*`, `/player/*`, `/admin/*` at edge; layouts add a second layer
2. ~~**Redesign invite flow — link-based with player self-selection**~~ ✅ Done — shareable join links live in Batch AG

3. **Fix video file size limit**

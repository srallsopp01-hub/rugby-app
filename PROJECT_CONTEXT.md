...
1. **Add `middleware.ts` — server-side auth guard**
   Protect `/coach/*`, `/player/*`, and `/admin/*` routes at the server layer. Without this, auth is client-side only. ~20–30 lines using `@supabase/ssr`. Required before any real users.

2. **Fix video file size limit**
   Check Supabase plan. If on free tier, either upgrade to Pro ($25/month) for 5 GB per-file limit, or migrate video storage to Cloudflare R2. R2 is the right long-term answer: no egress fees, no file size limit, ~$0.015/GB stored. Recommended approach: keep Supabase for auth + DB, move `match-videos` bucket to R2. Auth/access control would move from RLS to signed URLs generated by a server-side route.

3. **Redesign invite flow — link-based with player self-selection**
   Replace the email-specific invite with a shareable link system:
   - Coach generates a join link (no email required) from `/coach/team`
   - Link goes to `/invite/join?token=xxx` — anyone can open it
   - After auth (sign up or log in), new member sees squad roster and picks their player, or adds themselves as a new player
   - Coach sees pending approvals in `/coach/team` and can accept/reject
   - Accepted members get access; rejected members see a polite message
...
6. **Wire up the contact form** — connect `app/(marketing)/contact/page.tsx` to Resend email notification or a CRM/scheduling tool
7. **Add open graph images and brand assets to `public/`** — needed for social sharing previews
8. **Smoke-test full invite → player sync flow end-to-end** — fresh device, real invite, confirm player data loads
9. **Surface cloud sync status on coach home screen** — small "last synced X min ago" indicator; currently only visible in Settings
10. **Admin area auth guard** — add role check before any admin content renders

### Longer-term (don't prioritise yet)

- Cloudflare Stream / HLS transcoding (revisit at >50 hrs stored video or mobile streaming needed)

...
export default function TeamEventsPanel({
  ...
  const sheetPlayers = rosterRows.filter((r) => r.name.trim() !== "");

  const openPicker = (type: PendingNameFor) => {
    setPendingNameFor(type);
    setSelectedName(sheetPlayers[0]?.name.trim() ?? "");
  };
      ...
      {pendingNameFor ? (
        <div className="mt-3 space-y-3">
          ...
          {sheetPlayers.length > 0 ? (
            ...
            >
              {sheetPlayers.map((r) => (
                <option key={r.number} value={r.name.trim()}>
                  {r.number}. {r.name.trim()}
                </option>
              ))}
            </select>
          ) : (
            <p className="text-xs text-muted">No team sheet set up yet.</p>
          )}
          ...
          <div className="flex gap-2">
            <button
              ...
              disabled={sheetPlayers.length === 0}

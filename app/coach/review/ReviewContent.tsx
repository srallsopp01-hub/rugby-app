...
import type { ClipAnnotation, ClipComment, EventItem, RosterRow, VideoAnnotation } from "@/app/rugby-tagging/types";
...
function ClipCommentsPanel({
  ...
  const allComments = clip.comments ?? [];
  ...
  return (
    <div className="mt-3 border-t border-border pt-2">
      <p className="mb-1.5 text-[10px] font-semibold uppercase tracking-widest text-muted-2">Comments</p>
      {allComments.length > 0 && (
        <div className="mb-2 rounded-lg border border-border divide-y divide-border overflow-hidden">
          {allComments.map((c) => (
            <div key={c.id} className="px-2.5 py-2 flex gap-2">
      ...
      {allComments.length === 0 && (
        <p className="mb-2 text-xs text-muted-2">No comments yet.</p>
      )}
      <div className="flex gap-2">
        <textarea
          ...
          rows={2}
          className="flex-1 rounded-lg border border-border bg-panel px-2 py-1.5 text-xs text-foreground placeholder:text-muted-2 outline-none focus:border-border-light resize-none"
        />
        <button
          type="button"
          onClick={handlePost}
          disabled={!draft.trim()}
          className="self-end rounded-lg border border-border bg-panel-3 px-3 py-1.5 text-xs font-medium text-foreground transition-colors hover:border-border-light disabled:opacity-40 disabled:cursor-not-allowed"
        >
          Post
        </button>
      </div>
...
export default function ReviewPage() {
  ...
  const totalCommentCount = useMemo(
    () => clips.reduce((count, clip) => count + (clip.comments?.length ?? 0), 0),
    [clips]
  );
  ...
  const addClipComment = (clipId: number, comment: ClipComment) => {
    const next = clips.map((clip) =>
      clip.id !== clipId ? clip : { ...clip, comments: [...(clip.comments ?? []), comment] }
    );
    setClips(next);
    persistReviewState({ clips: next });
  };

  const deleteClipComment = (clipId: number, commentId: string) => {
    const next = clips.map((clip) =>
      clip.id !== clipId ? clip : { ...clip, comments: (clip.comments ?? []).filter((c) => c.id !== commentId) }
    );
    setClips(next);
    persistReviewState({ clips: next });
  };
      ...
      <div className="mx-auto max-w-[1900px] space-y-5">
        <PageHeader
          ...
          status={
            <>
              {totalCommentCount > 0 && (
                <StatusPill variant="neutral" size="md" title="Player comments on clips">
                  ...
                  {filteredClips.map((clip) => {
                    ...
                    return (
                      <div key={clip.id} className={`rounded-xl border p-3 ${isActive ? "border-border-light bg-panel-3" : "border-border bg-panel-2"}`}>
                        ...
                        {(clip.reactions?.length ?? 0) > 0 && (
                          <div className="mt-3 border-t border-border pt-2">
                            <p className="mb-1.5 text-[10px] font-semibold uppercase tracking-widest text-muted-2">Acknowledged</p>
                            <div className="flex flex-wrap gap-1.5">
                              {(clip.reactions ?? []).map((reaction) => {
                                ...
                                return (
                                  <StatusPill
                                    key={`${reaction.playerId}-${reaction.createdAt}`}
                        ...
                        <ClipCommentsPanel
                          clip={clip}
                          coachName={getTeam()?.coachName ?? "Coach"}
                          onAddComment={(comment) => addClipComment(clip.id, comment)}
                          onDeleteComment={(commentId) => deleteClipComment(clip.id, commentId)}
                        />
